...
public class MasterBukuPojo {
	...
	public Long version;
	public MasterGenrePojo id_genre_buku;
    ...
    public MasterBukuEntity toEntity() {
		...
		if(id_genre_buku != null && !id_genre_buku.id.equals("")){
			MasterGenreEntity entityRef = new MasterGenreEntity();
			entityRef.setKodeGenre(id_genre_buku.id);
...
//        entity.setJamupd(jamupd);
	...
	public static MasterBukuPojo fromEntity(MasterBukuEntity entity, int depthLevel) {
		...
		if (depthLevel > 0) {
			...
			pojo.id_genre_buku = MasterGenrePojo.fromEntity(entity.getGenreBuku());
